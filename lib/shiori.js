...
class Shiori {
  ...
  async build() {
    ...
    const { categories, tags } = await this.generateArchives(posts);

    // Generate pages
    await this.generatePosts(posts);
    await this.generateIndex(posts);
    await this.generatePages(posts, categories, tags);
  ...
  async generateIndex(posts) {
    ...
    const featuredPosts = posts.filter(post => post.featured)
      .slice(0, this.config.featured.limit);
    ...
    for (let page = 1; page <= totalPages; page++) {
      ...
      const html = await this.templateEngine.renderIndex(pagePosts, {
        ...
        featuredConfig: this.config.featured
      });
  ...
  async generateArchives(posts) {
    ...
    // Return categories and tags data
    return { categories, tags };
  }

  async generatePages(posts, categories, tags) {
    ...
    const categoriesHtml = await this.templateEngine.renderCategoriesIndex(categories);
    ...
    // Generate tags index
    const tagsHtml = await this.templateEngine.renderTagsIndex(tags);
