...
const ThemeRenderer = require('./theme-renderer');
...
class Shiori {
  constructor(config) {
    ...
    this.themeRenderer = new ThemeRenderer(config);
  ...
  async generateIndex(posts) {
    ...
    for (let page = 1; page <= totalPages; page++) {
      ...
      const html = await this.themeRenderer.renderIndex(pagePosts, {
        currentPage: page,
  ...
  async generateArchives(posts) {
    ...
    for (const [category, categoryPosts] of Object.entries(categories)) {
      const html = await this.themeRenderer.renderCategory(category, categoryPosts);
    ...
    for (const [tag, tagPosts] of Object.entries(tags)) {
      const html = await this.themeRenderer.renderTag(tag, tagPosts);
  ...
  async generatePages(posts) {
    ...
    const archiveHtml = await this.themeRenderer.renderArchive(posts);
    ...
    const categoriesHtml = await this.themeRenderer.renderCategoriesIndex();
    ...
    // Generate tags index
    const tagsHtml = await this.themeRenderer.renderTagsIndex();
